...
**来源判定(先做)**:会话可见的 agent 列表里有专家卡(如 agency-agents 的 232 个,住 `~/.claude/agents/`)→ **库选**;没有 → **现场生成 persona**(每个 = 角色名 + 单一目标 lens + 立场偏置 + 是否唱反调),用通用 subagent 派遣,persona 同样走第 4 步名单确认。两种来源的选角原则完全相同。

规模:轻型档 **3-5 个**,重型档 **6+**。原则:
...
// 动态填:议题 / 模式 / 专家名单 / 评论模式的靶子结论
// 双源:本机有专家库 → agentType 填专家卡名;无库(现场生成 persona)→ agentType 置 null
const ISSUE = `<议题>`
const MODE = `<讨论|评论|规划>`
const TARGET = `<评论模式:主 agent 初判结论;其他模式留空>`
const EXPERTS = [
  { name: 'Security Architect', agentType: 'Security Architect', lens: '安全', devil: true },
  { name: '架构鲁棒性专家', agentType: null, lens: '架构鲁棒性', devil: false },  // 生成 persona 示例
  // …名单
]
const SCHEMA = {
...
const build = (e) => `你是${e.name}。多视角${MODE}的一员。\n议题:\n${ISSUE}\n` +
...
const results = await parallel(EXPERTS.map(e => () =>
  agent(build(e), { label: `panel:${e.name}`, ...(e.agentType ? { agentType: e.agentType } : {}), schema: SCHEMA })
    .then(out => ({ expert: e.name, lens: e.lens, out }))
    .catch(() => ({ expert: e.name, lens: e.lens, out: null }))
))
...
要点:
- `agentType`(可选)让 Workflow 的 `agent()` 用**具体 agency 专家**(从 Agent 工具同源 registry 解析);**未装专家库时置 null**,走默认子 agent + persona 注入 prompt——角色深度略弱,但零外部依赖。
- `schema` 强制结构化产出,省去解析。
